/*
 * src/PixelFactory/FileFormatPNG.cpp
...
namespace EmEn::Base::PixelFactory
{
	...
	bool
	FileFormatPNG< pixel_data_t, dimension_t >::readStream (IO::ByteStream & stream, Pixmap< pixel_data_t, dimension_t > & pixmap) noexcept
	{
		pixmap.clear();
	...
	requires (std::is_arithmetic_v< pixel_data_t > && std::is_unsigned_v< dimension_t >)
	bool
	FileFormatPNG< pixel_data_t, dimension_t >::writeStream (IO::ByteStream & stream, const Pixmap< pixel_data_t, dimension_t > & pixmap, const WriteOptions & options) const noexcept
	{
		if ( !pixmap.isValid() )
		{
	...
	/* Explicit instantiations — the ONLY place libpng symbols enter the binary.
	 * libpng is an 8-bit API, and PixelFactory::FileIO already guards its dispatch with
	...
	template class FileFormatPNG< uint8_t, uint32_t >;
}