...
namespace EmEn::Base::PixelFactory
{
	 ...
	 * @note Every libpng call lives in FileFormatPNG.cpp, and this is a requirement rather than a
	 * matter of taste: an inline codec makes every consumer define libpng symbols in its OWN binary,
	 * where — on ELF, whose dynamic namespace is flat — they interpose the system libpng used by any
	 * library the process loads. Measured with cairo, FreeType and gdk-pixbuf behind CEF, which all
	 * ended up calling the vendored libpng instead of theirs. See
	...
	class FileFormatPNG final : public FileFormatInterface< pixel_data_t, dimension_t >
	{
		public:
			 ...
			 * @note Defined in FileFormatPNG.cpp, which explicitly instantiates this class for the
			 * pixel types libpng handles (8-bit only — FileIO already guards the dispatch with an
