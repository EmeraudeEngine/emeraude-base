/*
 * src/VertexFactory/ShapeGenerator.hpp
...
namespace EmEn::Base::VertexFactory::ShapeGenerator
{
	/**
		...
		requires (std::is_floating_point_v< vertex_data_t > && std::is_unsigned_v< index_data_t > )
	{
		...
		/* === Table (flat n-gon at Y = +crownHeight, facing the sky, +Y) === */
		{
			...
			{
				emitTriangle(
					tableRing[index], tableRing[0], tableRing[index + 1],
					tableNormal,
					tableUV[index], tableUV[0], tableUV[index + 1]
				);
			}
		}
		...
		requires (std::is_floating_point_v< vertex_data_t > && std::is_unsigned_v< index_data_t > )
	{
		...
		/* === Table (flat octagon at Y = +crownHeight, facing the sky, +Y) === */
		{
			...
			{
				emitTriangle(
					table[index], table[0], table[index + 1],
					tableNormal,
					tableUV[index], tableUV[0], tableUV[index + 1]
				);
			}
		...
		/* === Culet (flat octagon at Y = -pavilionDepth, opposite winding) === */
		{
			...
			{
				emitTriangle(
					culet[index + 1], culet[0], culet[index],
					culetNormal,
					culetUV[index + 1], culetUV[0], culetUV[index]
				);
			}
		}
		...
		requires (std::is_floating_point_v< vertex_data_t > && std::is_unsigned_v< index_data_t > )
	{
		...
		/* === Table (flat rectangle at Y = +crownHeight) === */
		{
			...
			emitTriangle(table[1], table[0], table[2], tableNormal, tableUV[1], tableUV[0], tableUV[2]);
			emitTriangle(table[2], table[0], table[3], tableNormal, tableUV[2], tableUV[0], tableUV[3]);
		}
		...
		/* === Culet (flat rectangle at Y = -pavilionDepth, opposite winding) === */
		{
			...
			emitTriangle(culet[3], culet[0], culet[2], culetNormal, culetUV[3], culetUV[0], culetUV[2]);
			emitTriangle(culet[2], culet[0], culet[1], culetNormal, culetUV[2], culetUV[0], culetUV[1]);
		}
		...
		requires (std::is_floating_point_v< vertex_data_t > && std::is_unsigned_v< index_data_t > )
	{
		...
		/* === Table === */
		{
			...
			for ( size_t index = 1; index + 1 < n; ++index )
			{
				emitTriangle(tableRingVec[index], tableRingVec[0], tableRingVec[index + 1], tableNormal, tableUV[index], tableUV[0], tableUV[index + 1]);
			}
		}
		...
		requires (std::is_floating_point_v< vertex_data_t > && std::is_unsigned_v< index_data_t > )
	{
		...
		for ( size_t ring = 0; ring <= rings; ++ring )
		{
			...
			/* Dome profile: parabolic height = h * t² for a gentle dome. */
		...
		/* === Flat base (Y = 0, facing sky = -Y) === */
		{
			...
			const auto baseNormal = Vec3::crossProduct(edge2, edge1).normalized();
			...
			for ( size_t index = 1; index + 1 < n; ++index )
			{
				emitTriangle(baseRing[index], baseRing[0], baseRing[index + 1], baseNormal, baseUV[index], baseUV[0], baseUV[index + 1]);
			}
		}
		...
		/* === Apex cap (last ring to apex point) === */
		{
			...
			for ( size_t index = 0; index < n; ++index )
			{
				...
				const auto normal = Vec3::crossProduct(lastRing[next] - lastRing[index], apex - lastRing[index]).normalized();
				...
				emitTriangle(apex, lastRing[index], lastRing[next], normal, uvs[2], uvs[0], uvs[1]);
			}
		}
