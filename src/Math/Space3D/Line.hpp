...
namespace EmEn::Base::Math::Space3D
{
	...
	class Line final
	{
		public:
			...
			explicit
			constexpr
			Line (const Vector< 3, precision_t > & direction) noexcept
			 ...
			 */
			constexpr
			Line (const Point< precision_t > & origin, const Vector< 3, precision_t > & direction) noexcept
				: m_origin(origin),
				m_direction(direction.normalized())
			{
			...
			{
				m_origin.reset();
				m_direction = Vector< 3, precision_t >::positiveX();
			}
		...
		private:
			...
			Point< precision_t > m_origin{0, 0, 0};
			Vector< 3, precision_t > m_direction{1, 0, 0};
	};
}
