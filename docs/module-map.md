...
| pixel | `emeraude::base::pixel` | OBJECT | PNG, JPEG, TIFF, WebP, Freetype | DONE (**became OBJECT in 2026-08**: PixelFactory stays all templates, but the third-party codecs — `FileFormatPNG/Jpeg/TIFF/WebP`, `Font::readTrueTypeFile` — are compiled here and explicitly instantiated, so no consumer inlines libpng/libjpeg/libtiff/libwebp/FreeType into its own binary. See [`../cmake/HideThirdPartyExports.cmake`](../cmake/HideThirdPartyExports.cmake)). ⚠️ **WebP (2026-09-18)** decodes to plain RGB/RGBA and is NOT a GPU container: it takes the ordinary `Pixmap` path like PNG, never the block-compressed KTX2 one. It has no grayscale mode, so a read always yields RGB/RGBA and a write refuses a grayscale pixmap. ⚠️⚠️ Its **encoder needs `libsharpyuv.a`, a SEPARATE archive** listed after `libwebp.a` in `SetupWebP.cmake` — every object compiles without it and the link then fails on `SharpYuvInit`/`SharpYuvConvert`/`SharpYuvGetConversionMatrix`, which a decode-only test never reaches. |
