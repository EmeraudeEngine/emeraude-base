...
| 20-type Variant (exception-free `as*`) | `Variant.hpp/.cpp` | ✅ 10 tests (`test_Variant.cpp`): null/reset/copy/set, per-type round-trips, Type/index invariant, `to_cstring`, mismatch→default+logged. Confirmed it should stay (justified `-fno-exceptions`-safe facade over `std::variant`, ~46 consumers in Animations + Scene components; bare STL would push UB/terminate risk to every call site). Impl slimmed: 20 duplicated `asXxx()` → one private templated `as<T>()`, API unchanged, `cerr→Logging`. | ✅ | type/index invariant + mismatch paths |
