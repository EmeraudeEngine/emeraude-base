...
| ↳ Real correctness gaps (markers) | ⬜ not started | TriangleGenerator, OrientedCuboid, Processor mask, ShapeBuilder. **NEW (found 2026-06-03 while doing the colored stencil):** the *pixmap-source* `Processor::stencil()` overloads ignore their mask (the `FIXME` at Processor.hpp:965 — they plain-copy) **and** call `blendPixel(destinationIndex+pixel, row, …)` passing a full index as the X coord → wrong target indexing. Pre-existing; separate fix (the colored variant is now correct and can be the reference). |
