...
| ↳ Finish capabilities | 🟦 4/5 done | ✅ **Pixel colored stencil** (was a `return false` stub → composites a solid colour through a grayscale mask: white passes, black blocks, grey = AA via blend opacity; test `coloredStencilRespectsMask`). ✅ **Hash enum** (removed never-implemented `SHA1`, exposed implemented `SHA512`; round-trip + FIPS-180-4 SHA512 KAT tests). ✅ **platform compiler-id** (`PLATFORM_COMPILER` + version macros & constexpr mirrors, Clang-before-GCC; `test_Platform`). ✅ **Debug ns timer** (was Linux-only + `tv_nsec`-only → now delegates to `Time::processCPUTimeNanoseconds()`, cross-platform + full-ns; `test_Debug`). ⬜ **Animation CubicSpline** (tangent storage) — the one remaining, larger item. |
| ↳ Real correctness gaps (markers) | 🟦 stencil done | **Stencil family FIXED 2026-06-03:** the pixmap-source `Processor::stencil()` overloads now respect the grayscale mask (white passes / black blocks / grey = AA) and use the index-based `blendPixel(index, colour, mode, opacity)` — the former `blendPixel(index, row, …)` passed a full flat index as the X coord (wrong target indexing) and ignored the mask (FIXME). The core gained an `opacity` param so the convenience overload (which forwarded 6 args to a 5-param core — uncompilable had it ever been called) now matches. Test `pixmapStencilRespectsMask` (core + convenience). **Still open:** TriangleGenerator (remove internal triangles), OrientedCuboid (bounds from vertices), Processor mask-value checks, ShapeBuilder review. |
